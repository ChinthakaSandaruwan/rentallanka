...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        if ($phone === '' || !in_array($status, ['active','inactive','banned'], true)) {
            redirect_with_message('owner_management.php', 'Invalid input', 'error');
        }
        $stmt = db()->prepare("INSERT INTO users (email, phone, role, status) VALUES (?,?,?,?)");
        $stmt->bind_param('ssss', $email, $phone, $role, $status);
        if (!$stmt->execute()) {
            redirect_with_message('owner_management.php', 'Create failed: ' . $stmt->error, 'error');
        }
        redirect_with_message('owner_management.php', 'Owner created');
    }
    ...
    if ($action === 'update') {
        ...
        if (!$stmt->execute()) {
            redirect_with_message('owner_management.php', 'Update failed: ' . $stmt->error, 'error');
        }
        redirect_with_message('owner_management.php', 'Owner updated');
    }
}
...
if (($_GET['action'] ?? '') === 'delete') {
    ...
    if ($user_id > 0) {
        ...
        if (!$stmt->execute()) {
            redirect_with_message('owner_management.php', 'Delete failed: ' . $stmt->error, 'error');
        }
        redirect_with_message('owner_management.php', 'Owner deleted');
    } else {
        redirect_with_message('owner_management.php', 'Invalid owner id', 'error');
    }
}
...
<body>
  ...
  <h3><?php echo $editItem ? 'Edit Owner' : 'Create Owner'; ?></h3>
  <form method="post">
    ...
    <?php if ($editItem): ?>
      <a class="button" href="owner_management.php">Cancel</a>
    <?php endif; ?>
  </form>
    ...
    <tbody>
      <?php foreach ($list as $row): ?>
        <tr>
          ...
          <td>
            <a class="button" href="owner_management.php?action=edit&user_id=<?php echo (int)$row['user_id']; ?>">Edit</a>
            <a class="button" href="owner_management.php?action=delete&user_id=<?php echo (int)$row['user_id']; ?>" onclick="return confirm('Delete this owner?');">Delete</a>
          </td>
        </tr>
      <?php endforeach; ?>
    </tbody>
