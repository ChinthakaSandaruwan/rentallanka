...
require_once __DIR__ . '/../public/includes/auth_guard.php'; require_role('admin');
...
$recipients = [];
$selRole = $_GET['role'] ?? '';
if (!in_array($selRole, ['admin','owner','customer'], true)) { $selRole = 'customer'; }
$q = trim($_GET['q'] ?? '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($to_user <= 0 || $title === '' || $message === '' || !in_array($type, ['system','rental','payment','other'], true)) {
    redirect_with_message($GLOBALS['base_url'] . '/admin/send_notification.php', 'Please complete all fields.', 'error');
  }
  ...
  $u->close();
  if (!$exists) {
    redirect_with_message($GLOBALS['base_url'] . '/admin/send_notification.php', 'Recipient not found.', 'error');
  }
  $ins = db()->prepare('INSERT INTO notifications (user_id, title, message, type, rental_id, property_id, is_read) VALUES (?, ?, ?, ?, NULL, NULL, 0)');
  if (!$ins) {
    redirect_with_message($GLOBALS['base_url'] . '/admin/send_notification.php', 'Server error preparing insert.', 'error');
  }
  $ins->bind_param('isss', $to_user, $title, $message, $type);
  if ($ins->execute()) {
    $ins->close();
    redirect_with_message($GLOBALS['base_url'] . '/admin/notification.php', 'Notification sent.', 'success');
  } else {
    $ins->close();
    redirect_with_message($GLOBALS['base_url'] . '/admin/send_notification.php', 'Failed to send notification.', 'error');
  }
}
?>
<!doctype html>
...
<div class="container py-4">
  ...
  <form method="post" class="row g-3">
    ...
    <div class="col-12">
      ...
      <a href="<?= $base_url ?>/admin/notification.php" class="btn btn-secondary">Back</a>
    </div>
  </form>
</div>
