  ...
  if (provSel && distSel && citySel) {
    ...
    fetch(baseUrl + '?geo=provinces')
      ...
      .then(list=>{
        ...
        const pid = current.province_id || provSel.value;
        if (pid) {
          return fetch(baseUrl + '?geo=districts&province_id=' + encodeURIComponent(pid))
            .then(r=>r.json())
            .then(list=>{
              fillSelect(distSel, list.map(x=>({value:x.district_id,label:x.name})), 'Select district', current.district_id);
              const did = current.district_id || distSel.value;
              if (did) {
                return fetch(baseUrl + '?geo=cities&district_id=' + encodeURIComponent(did))
                  .then(r=>r.json())
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      if (price) {
        ...
        if (isNaN(val) || val <= 0) errors.push('Price per month must be a number greater than 0.');
      }

      // Integers non-negative
      ['bedrooms','bathrooms','living_rooms'].forEach(name => {
