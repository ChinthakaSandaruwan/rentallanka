...
$q = db()->prepare("SELECT room_id FROM rooms WHERE room_id=? AND owner_id=? LIMIT 1");
$q->bind_param('ii', $rid, $uid);
$q->execute();
        ...
        if ($action === 'upload_images') {
            if (!empty($_FILES['gallery_images']['name']) && is_array($_FILES['gallery_images']['name'])) {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    $fname = 'room_' . $rid . '_' . ($i + 1) . '_' . time() . '.' . $ext;
                    $dest = $dir . '/' . $fname;
            ...
            redirect_with_message($GLOBALS['base_url'] . '/owner/room_image_edit.php?id=' . (int)$rid, 'Images uploaded', 'success');
            ...
            redirect_with_message($GLOBALS['base_url'] . '/owner/room_image_edit.php?id=' . (int)$rid, 'Primary image updated', 'success');
            ...
            if ($image_id > 0) {
                ...
                if ($row) {
                    ...
                    if ($fname) {
                        ...
                        if (is_file($full) && strpos(realpath($full) ?: '', realpath(dirname(__DIR__) . '/uploads/rooms')) === 0) {
                            @unlink($full);
                        }
                    }
                    if ((int)$row['is_primary'] === 1) {
                        $nx = db()->prepare('SELECT image_path, image_id FROM room_images WHERE room_id=? ORDER BY is_primary DESC, image_id DESC LIMIT 1');
                        $nx->bind_param('i', $rid);
                        $nx->execute();
                        ...
                        if ($newPath && $newId) {
                            ...
                            $clr->execute();
                            $clr->close();
                            $sp = db()->prepare('UPDATE room_images SET is_primary=1 WHERE room_id=? AND image_id=?');
                            $sp->bind_param('ii', $rid, $newId);
                            $sp->execute();
                            $sp->close();
                        }
                    }
            ...
            redirect_with_message($GLOBALS['base_url'] . '/owner/room_image_edit.php?id=' . (int)$rid, 'Image deleted', 'success');
        }
    }
}
...
<div class="container py-4">
  <div class="d-flex align-items-center justify-content-between mb-3">
    <h1 class="h3 mb-0">Manage Room Images</h1>
    <a href="room_edit.php?id=<?php echo (int)$rid; ?>" class="btn btn-outline-secondary btn-sm">Back</a>
  </div>
  <?php if ($flash): ?>
      ...
      <div class="card mb-4">
        ...
        <div class="card-body">
          <div class="mb-3">
            ...
            <div class="fw-semibold"><?php echo 'ROOM-' . str_pad((string)$rid, 6, '0', STR_PAD_LEFT); ?></div>
          </div>
          <?php
            $pimg = '';
            try {
              $sp = db()->prepare('SELECT image_path FROM room_images WHERE room_id=? ORDER BY is_primary DESC, image_id DESC LIMIT 1');
              $sp->bind_param('i', $rid);
              $sp->execute();
