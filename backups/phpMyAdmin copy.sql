...
CREATE TABLE IF NOT EXISTS `payment_slips` (
  ...
  CHECK ((`rental_id` IS NOT NULL AND `booking_id` IS NULL) OR (`rental_id` IS NULL AND `booking_id` IS NOT NULL))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `cart_items` (
  ...
  CHECK ((`unit_id` IS NOT NULL AND `property_id` IS NULL) OR (`unit_id` IS NULL AND `property_id` IS NOT NULL))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
