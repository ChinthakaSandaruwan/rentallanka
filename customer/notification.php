...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($nid > 0) {
    if ($action === 'read') {
      ...
      $ok = $st->execute();
      $st->close();
      redirect_with_message($GLOBALS['base_url'] . '/customer/notification.php', $ok ? 'Marked as read' : 'Unable to mark as read', $ok ? 'success' : 'error');
    } elseif ($action === 'unread') {
      $st = db()->prepare('UPDATE notifications SET is_read=0 WHERE notification_id=? AND user_id=? LIMIT 1');
      $st->bind_param('ii', $nid, $uid);
      $ok = $st->execute();
      $st->close();
      redirect_with_message($GLOBALS['base_url'] . '/customer/notification.php', $ok ? 'Marked as unread' : 'Unable to mark as unread', $ok ? 'success' : 'error');
    } elseif ($action === 'delete') {
      $st = db()->prepare('DELETE FROM notifications WHERE notification_id=? AND user_id=? LIMIT 1');
      $st->bind_param('ii', $nid, $uid);
      $ok = $st->execute();
      $st->close();
      redirect_with_message($GLOBALS['base_url'] . '/customer/notification.php', $ok ? 'Notification deleted' : 'Unable to delete notification', $ok ? 'success' : 'error');
    }
  }
  redirect_with_message($GLOBALS['base_url'] . '/customer/notification.php', 'Invalid action', 'error');
}
...
<div class="container py-4">
  <div class="d-flex align-items-center justify-content-between mb-3">
    ...
    <a href="<?= $base_url ?>/customer/send_notification.php" class="btn btn-sm btn-primary"><i class="bi bi-send me-1"></i>Send Notification</a>
  </div>
  <?php if (!empty($flash)): ?>
